...
cdnoded config chain-id oasis-1

# Create a new wallet named "me"
cdnoded keys add me

# Initialize a new genesis.json file
cdnoded init me --chain-id oasis-1 > /dev/null 2>&1 

# Add your freshly created account to the new chain genesis
cdnoded add-genesis-account me 1000000000ubir > /dev/null 2>&1 

# Generate the genesis transaction to create a new validator
cdnoded gentx me 100000000ubir --chain-id oasis-1 --commission-rate 0.1 --commission-max-rate 0.2 --commission-max-change-rate 0.01 > /dev/null 2>&1
...
sed -i 's/stake/ubir/g' ~/.cdnode/config/genesis.json > /dev/null 2>&1

# Edit app.toml to set the minimum gas price
sed -i 's/minimum-gas-prices\ =\ \"\"/minimum-gas-prices\ =\ \"0.0025ubir\"/g' ~/.cdnode/config/app.toml > /dev/null 2>&1
